...
对标对象是 [Borderless Gaming](https://github.com/Codeusa/Borderless-Gaming)(下称 BG)。Reframe 要解决的,是它在多显示器与串流场景下几个一用就疼的点:

| Borderless Gaming | Reframe |
|---|---|
| 位置写死在每个游戏里。改一次布局,要逐个 profile 手动改。 | **布局是一等公民。** 命名布局含若干分区,profile 只引用 `布局 + 分区`。改一处布局,所有引用它的游戏跟着变。 |
| 尺寸是绝对像素——换显示器(比如 VDD 串流)就错位。 | **分区以 0~1 比例存储**,按窗口**当前**所在的显示器解析。7680 宽屏的 ⅔ 是 5120 像素;到更小的串流屏上自动换算。 |
| 没有按显示器分支:同一个游戏在不同屏上无法有不同行为。 | 每个 profile 持有一张**规则表**(自上而下第一条命中的显示器生效):`7680×2160 → 套游戏区`、`其它任意屏 → 铺满`。 |
| 改配置要重启才生效。 | **热重载。** UI 改即生效,外部手改 `config.json` 也会被监听并重新加载。 |
| 3 秒轮询,慢。 | **事件驱动**(WinEvent 钩子:窗口出现 / 标题变化 / 被游戏改回),配一条低频兜底轮询。 |
| 去边框不可逆。 | 改动前**快照**原始样式与位置,禁用 profile 或停止引擎时自动还原。 |
...
把这份提权用在哪,说明白:**Reframe 不注入代码,不读写游戏内存。** 它只调用标准 Win32 窗口 API——用 `SetWindowLongPtr` 砍掉标题栏/边框样式,用 `SetWindowPos` 移动和调整窗口尺寸。这正是 Borderless Gaming 多年来沿用的保守做法。(详见 [FAQ](#faq)。)
...
完整设计——数据模型、引擎管线、与 Borderless Gaming 的逐项对比——见 [DESIGN.md](DESIGN.md)。
...
**用在反作弊游戏上安全吗?**
Reframe 只通过有文档的 Win32 API(`SetWindowLongPtr`、`SetWindowPos`)改窗口的**样式**与**位置**。它从不注入代码、从不读写其它进程的内存、从不碰游戏文件。这与 Borderless Gaming 多年沿用的手法相同。但话说回来,没有任何第三方工具能担保某个反作弊会作何反应——请自行判断。
...
- 部分 BG 高级选项有意未做(保持客户区、跨所有屏、隐藏任务栏、移除菜单等),详见 [DESIGN.md](DESIGN.md) §4 状态列。

## 参与贡献

欢迎提 issue 和 PR——bug 反馈、功能点子、使用疑问,都有帮助。

**尤其欢迎翻译。** Reframe 正在把 UI 文案迁到 `.resw` 资源,做正经的多语言支持,所以现在正是把它带到新语言的好时机。想翻译的话,开个 issue,我们一起把资源文件理清楚。

提 PR 时,请让 **Core** 层保持零 UI 依赖(摆放数学住在那儿,有单测覆盖),并在推送前跑一遍 `dotnet test Tests\Reframe.Core.Tests.csproj`。

## 致谢

- [**Borderless Gaming**](https://github.com/Codeusa/Borderless-Gaming) —— 本项目要扩展的原作,也是「只改样式与位置、不注入」这套保守做法的来源。
